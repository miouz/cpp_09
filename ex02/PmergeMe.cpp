 ...
 */
void initChainsABRested(std::vector<int>& nums, size_t numsInPair, std::vector<int>& chainAOriginal,
						 std::vector<int>& chainBOriginal, std::vector<int>& rested)
{
 ...
 */
void updateMainchain(std::vector<int>& nums, size_t numsInPair, size_t NthPairInB,
					 std::vector<int>& chainAOriginal, size_t i)
{
 ...
 */
std::vector<int>::iterator findAPosOnMain(std::vector<int>&nums, std::vector<int>& chainAOriginal,
...
void insertion(std::vector<int>& nums, std::size_t numsInPair)
{
	...
	for(size_t i = 0; i < JACOBSTHAL_MAX; i++)
	{
		if (jacobsthal[i] == 1)
			continue;
		std::size_t NthPairInB = jacobsthal[i];
		...
		for (size_t toInsertNthPairInB = NthPairInB; toInsertNthPairInB > jacobsthal[i - 1]; toInsertNthPairInB--)
		{
			...
			std::vector<int>::iterator toCompareUntilOnMain = findAPosOnMain(nums, chainAOriginal, toInsertNthPairInB, newAddedOnMainchain, numsInPair);
